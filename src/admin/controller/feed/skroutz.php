...
class ControllerFeedSkroutz extends Controller {
  ...
  public function index() {
    $this->load->language('feed/skroutz');
    ...
    if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
      $this->model_setting_setting->editSetting('skroutz', $this->request->post);
    ...
    $data['breadcrumbs'][] = array(
      ...
      'href' => $this->url->link('feed/skroutz', 'token=' . $this->session->data['token'], true)
    );

    $data['action'] = $this->url->link('feed/skroutz', 'token=' . $this->session->data['token'], true);
    ...
    $data['data_feed'] = HTTP_CATALOG . 'index.php?route=feed/skroutz';

    $data['header'] = $this->load->controller('common/header');
    $data['column_left'] = $this->load->controller('common/column_left');
    $data['footer'] = $this->load->controller('common/footer');

    $this->response->setOutput($this->load->view('feed/skroutz', $data));
  }

  protected function validate() {
    if (!$this->user->hasPermission('modify', 'feed/skroutz')) {
